...
midi_output = mido.open_output("PythonToAbleton 2") # This will open the default output

note_buffer = {}#{"1":{"note":123,"ts":3434}

NOTE_TIMEOUT = 5.0  # seconds
...
def osc_to_midi_note_on(address, *args):
    ...
    try:
        ...
        if (touch_id in note_buffer and note_buffer[touch_id]["note"] != note):
            msg = mido.Message('note_off', note=note_buffer[touch_id]["note"], velocity=0, channel=channel)
        ...
        note_buffer[touch_id]["ts"] = time.time()  # Store timestamp
...
def osc_to_midi_cc(address, *args):
    global note_buffer  # Add this line
    try:
        ...
        if channel == 3:
           # msg = mido.Message('control_change', control=0, value=0, channel=4)
            ...
            midi_output.send(msg)
            note_buffer = {}  # This now clears the global note_buffer
...
def stale_note_watcher():
    while True:
        now = time.time()
        stale_ids = []
        for touch_id, info in list(note_buffer.items()):
            if now - info.get("ts", now) > NOTE_TIMEOUT:
                stale_ids.append(touch_id)
        for touch_id in stale_ids:
            note = note_buffer[touch_id]["note"]
            channel = 0  # Default channel, adjust if needed
            msg = mido.Message('note_off', note=note, velocity=0, channel=channel)
            midi_output.send(msg)
            print(f"Auto Sent MIDI Note Off: {msg} {touch_id}")
            del note_buffer[touch_id]
        time.sleep(0.2)  # Check 5 times per second

# Start watcher thread
threading.Thread(target=stale_note_watcher, daemon=True).start()
