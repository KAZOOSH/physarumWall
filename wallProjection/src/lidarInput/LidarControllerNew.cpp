...
LidarControllerNew::LidarControllerNew()
{
}
...
void LidarControllerNew::setup(ofJson settings)
{
    ...
    {
        sensors.push_back(shared_ptr<Ms200ReceiverSimple>(new Ms200ReceiverSimple()));
        ofJson stats = ofJson();
        stats["lidar"] = lidar;
        for (auto &screen : settings["screens"])
        {
...
void LidarControllerNew::update()
{
...
void LidarControllerNew::registerInputs(shared_ptr<GenericInput> input)
{
    for (auto &sensor : sensors)
    {
        ofAddListener(input->interactionStart, this, &LidarControllerNew::onTouchDown);
        ofAddListener(input->interactionMove, this, &LidarControllerNew::onTouchMove);
        ofAddListener(input->interactionEnd, this, &LidarControllerNew::onTouchUp);
    }
}

void LidarControllerNew::onTouchMove(ofTouchEventArgs &ev)
{
    touches[ev.id] = ev;
...
void LidarControllerNew::mapTouchToTexCoords(ofTouchEventArgs &ev)
{
...
std::vector<PointCluster> LidarControllerNew::clusterPointsOptimized(std::vector<ofVec2f> points, double maxDistance)
{
...
void LidarControllerNew::updateTexture()
{
    debugFbo.begin();
    ofClear(0, 0);
...
void LidarControllerNew::onTouchUp(ofTouchEventArgs &ev)
{
    touches.erase(ev.id);
    mapTouchToTexCoords(ev);
...
void LidarControllerNew::onTouchDown(ofTouchEventArgs &ev)
{
    touches[ev.id] = ev;
