...
void LidarController::setup(ofJson settings) {
	...
	for (auto & lidar : settings["lidars"]) {
		...
		ofJson stats = ofJson();
		stats["lidar"] = lidar;
		bool isMatchingScreen = false;
		for (auto & screen : settings["screens"]) {
			if (screen["id"] == lidar["screenId"]) {
				stats["screen"] = screen;
				isMatchingScreen = true;
			}
		}
		if (!isMatchingScreen) {
			ofLogError("no matching screen  - " + lidar["screenId"].get<string>() + " - for lidar found");
		} else {
...
std::vector<PointCluster> LidarController::clusterPointsOptimized(std::vector<ofVec2f> points, double maxDistance) {
	...
	std::sort(points.begin(), points.end(), [](const ofVec2f & a, const ofVec2f & b) { return a.x < b.x; });
