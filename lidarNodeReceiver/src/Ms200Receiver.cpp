...
Ms200Receiver::Ms200Receiver(int port, string environmentSettingsFile)
{
    // load environment
    ofBuffer buffer = ofBufferFromFile(environmentSettingsFile.c_str());
    if (buffer.size())
    {
...
void Ms200Receiver::updateClusters(map<u_int64_t, Cluster> &clusters, const map<int, LidarRawSample> &samples, map<int, LidarRawSample> &environment)
{
    ...
    for (auto& e:vEnd)
    {
        auto pos = polarToCartesian(clustersOld[e].meanAngle,clustersOld[e].meanDist);
        ofTouchEventArgs t = ofTouchEventArgs(ofTouchEventArgs::up, pos.x,pos.y,e);
        t.width = clustersOld[e].radius*2;
        t.height = clustersOld[e].radius*2;
        interactionEnd.notify(t);
    }

    for (auto& e:vEnter)
    {
        auto pos = polarToCartesian(clusters[e].meanAngle,clusters[e].meanDist);
    ...
    for (auto& e:vMove)
    {
        auto posOld = polarToCartesian(clustersOld[e].meanAngle,clustersOld[e].meanDist);
        auto pos = polarToCartesian(clusters[e].meanAngle,clusters[e].meanDist);
